...
class PoliticsNYTimes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            error: null,
    ...
    componentDidMount() {
        fetch('http://ec2-3-84-36-254.compute-1.amazonaws.com:5000/api/nytimes/politics')
        .then(res => res.json())
        .then( (result) => {
                this.setState({ results: result.results, isLoaded: true }, 
                () => console.log('NYTimes politics Result fetched...', result.results))
              })
  }
...
export default PoliticsNYTimes;
