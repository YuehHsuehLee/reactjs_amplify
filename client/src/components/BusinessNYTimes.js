...
class BusinessNYTimes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            error: null,
    ...
    componentDidMount() {
        fetch('http://ec2-3-84-36-254.compute-1.amazonaws.com:5000/api/nytimes/business')
        .then(res => res.json())
        .then( (result) => {
                this.setState({ results: result.results, isLoaded: true }, 
                () => console.log('NYTimes Business Result fetched...', result.results))
              })
  }
...
render() {
    ...
    const temp1 = temp.filter(article => article.section == 'business');
    const articleToRender = temp1.slice(0, 10);
...
export default BusinessNYTimes;
