...
class PoliticsGuardian extends Component {
    constructor(props) {
        super(props);
        this.state = {
            error: null,
    ...
    componentDidMount() {
        fetch('http://ec2-3-84-36-254.compute-1.amazonaws.com:5000/api/guardian/politics')
        .then(res => res.json())
        .then( (result) => {
                this.setState({ results: result.response.results, isLoaded: true }, 
                () => console.log('Guardian politics Result fetched...', result.response.results))
              })
  }
...
export default PoliticsGuardian;
